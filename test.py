...
for i in range(2000):
    ...
    s_t = np.hstack((ob.angle, ob.track, ob.trackPos, ob.speedX, ob.speedY, ob.speedZ, ob.wheelSpinVel/100.0, ob.rpm)) #state
    ...
    if np.mod(i, 3) == 0:
        if (train_indicator):
            print("saving model")
            torch.save(actor.state_dict(), './DDPG_model/actormodel.pth')
            torch.save(critic.state_dict(), './DDPG_model/criticmodel.pth')
