...
class TorcsEnv:
    ...
    def __init__(self, vision=False, throttle=False, gear_change=False):
        ...
        if self.vision is True:
            ...
            os.system('sudo torcs -nofuel -nolaptime &')
        time.sleep(0.5)
    ...
    def step(self, u):
       #print("Step")
        ...
        spz = np.array(obs['speedZ'])
        damage = np.array(obs['damage'])
        rpm = np.array(obs['rpm'])
        laptime = np.array(obs['lastLapTime'])


        # ## GTS-ETH reward ###############
        # cw = 0.005
        # progress = (obs['distFromStart'] - obs_pre['distFromStart']) / (obs['curLapTime'] - obs_pre['curLapTime'])
        # reward = progress

        # # collision detection
        # if obs['damage'] - obs_pre['damage'] > 0:
        #     #reward = -1
        #     reward = progress - cw * (sp**2+spy**2+spz**2)
